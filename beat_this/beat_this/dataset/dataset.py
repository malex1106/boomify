...
class BeatTrackingDataset(Dataset):
    ...
    def _load_dataset_item(self, item_name):
        ...
        for aug_filename in precomputed_augmentation_filenames(self.augmentations):
            spect_exists = (self.spect_basepath / item_name / aug_filename).exists()
            boom_spect_exists = (self.spect_basepath / f"{dataset}_boom/{remainder}" / aug_filename).exists()

            if (f"{remainder}/{aug_filename[:-4]}") not in self.spects.get(
                dataset, ()
            ) and not (spect_exists or boom_spect_exists):
                print(
    ...
    def __getitem__(self, index):
        if isinstance(index, (int, np.int64)):  # when index is a single int
            ...
            item = augment_pitchtempoboom(item, self.augmentations)
...
class BeatDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        if not set(augmentations.keys()).issubset({"mask", "pitch", "boom", "tempo"}):
