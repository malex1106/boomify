...
def augment_pitchtempoboom(item, augmentations):
    """
    ...
    augmentations: dict
        ...
        - 'boom': A dictionary with 'noise' and 'num' specifying the noise level and the number of variations per file.
    ...
    if "tempo" in augmentations:
        available_augmentations.append("tempo")
    if "boom" in augmentations:
        available_augmentations.append("boom")

    # Handle multiple augmentation selections
    if available_augmentations:
        selected_augmentation = np.random.choice(available_augmentations)
        
        if selected_augmentation == "pitch":
            item = augment_pitch(item, augmentations["pitch"])
        elif selected_augmentation == "tempo":
            item = augment_tempo(item, augmentations["tempo"])
        elif selected_augmentation == "boom":
            item = augment_boom(item, augmentations["boom"])
...
def augment_boom(item, noise_params):
    num_aug = np.random.randint(noise_params["num"] + 1)
    item = boom_filename(item, num_aug, noise_params)
    item = boom_annotations(item, num_aug, noise_params)
    return item
...
def boom_annotations(item, noise, noise_params):
    return item
...
def boom_filename(item, num_aug, noise_params):
    spect_path = item["spect_path"]
    noise = noise_params["noise"]
    if num_aug:
        stem = spect_path.stem + f"_bs{noise}_{num_aug}"
        spect_path = spect_path.with_stem(stem)
    return {**item, "spect_path": spect_path}
...
def precomputed_augmentation_filenames(augmentations, ext="npy"):
    ...
    for method, params in augmentations.items():
        if method == "pitch":
            ...
            for cur_aug in range(params["num"] + 1):
                if cur_aug == 0:
                    continue
                filenames.append(f"track_bs{params['noise']}_{cur_aug}.{ext}")
    return filenames
